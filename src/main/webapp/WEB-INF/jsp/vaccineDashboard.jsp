...
function getDistrict(){
	...
	$.ajax({
	    ...
	    dataType : "json",
	    url: 'http://localhost:8080/getDistrictByStateId',
	    data: { 
	        'stateid': stateid	        
	    },
							...
							<div class="card-body">
								<ul class="nav nav-tabs" id="myTab" role="tablist">
									<li class="nav-item" role="presentation"><a
										...
										aria-selected="true">Search By PIN</a></li>
									<li class="nav-item" role="presentation"><a
										class="nav-link" id="profile-tab" data-bs-toggle="tab"
										href="#profile" role="tab" aria-controls="profile"
										aria-selected="false">Search By District</a></li>
									<!-- <li class="nav-item" role="presentation"><a
														...
														<div class="card-content">
															<div class="card-body">
																<form class="form form-horizontal" id="createReport"
																	action="/searchByPincode" method="get">
																	<div class="form-body">
																		<div class="row">
																			<div class="col-md-4">
																				<label>Pincode</label>
																	...
																	<tbody>
																		...
																		<c:choose>
																			<c:when test="${responseDto.size() != 0}">
																				<c:forEach var="list" items="${responseDto}">
																					<c:set var="string1"
																						value="${fn:replace(list.slots, '[', '')}" />
																					<c:set var="string2"
																					...
																					<tr>
																						<td>${list.date}</td>
																						<td>${list.name}-${list.pincode}</td>
																						<td style="color: green;"><b>${list.available_capacity}</b></td>
																						<td>${list.vaccine}<br> Age: <b
																							style="color: red;">${list.min_age_limit}+</b>
																						</td>

																						<td>${string4}<%-- <button type="button" class="btn btn-secondary"
																								data-bs-toggle="tooltip"
																								data-bs-placement="bottom" title="${string4}">
																								Check Slot</button> --%>
																						</td>
																					</tr>
																				</c:forEach>
																			...
																			<c:otherwise>
																				<tr>
																					<td colspan="5" align="center"><br>No
																						Slots Available</td>
																				</tr>
																			</c:otherwise>
																		</c:choose>
